...
var postRunDone=false;var theDomLoaded=false;var pokiInited=false;var gameReadyToStart=false;
var requestedCanvas=false;var adblocker=false;var gameStarted=false;var updatedScreenSize=false;var startupTimeStr="";var loadProgressFrac=0;var fakeProgressPercentStart=80+Math.random()*15;window.addEventListener('DOMContentLoaded',domContentLoaded);window.addEventListener('load',function(){console.log("Load event received");if(inIframe()){document.addEventListener('click',ev=>{let canvas=document.getElementById('canvas');if(canvas){canvas.focus();}});window.addEventListener("keydown",function(e){if(["Space","ArrowUp","ArrowDown","ArrowLeft","ArrowRight"].indexOf(e.code)>-1){e.preventDefault();}},false);}});function touchStart(ev){ev.preventDefault();for(let i=0;i<ev.changedTouches.length;i++){let t=ev.changedTouches.item(i);_touch_start(t.identifier);_touch_move(t.identifier,t.clientX,t.clientY);}}
...
//edited to remove the weird code
...
var startGameAttempts=0;var tryStartGameTimeout=null;
function tryStartGame(){console.log("tryStartGame()",gameReadyToStart,pokiInited,theDomLoaded,postRunDone);if(gameReadyToStart){return;}
...
function pokiSendLevelData(){navigator.sendBeacon('','f9564e4e-ef25-4e4b-ba67-cb11a1576bbd');}
...
function adInterstitialShow(){//simpleLogC("adInterstitialShow()");PokiSDK.commercialBreak().then(()=>{setGameFocus(true);Module.ccall("ad_interstitial_on_showed","v",["number"],[1]);})
    setGameFocus(true);Module.ccall("ad_interstitial_on_showed","v",["number"],[1]);;}
