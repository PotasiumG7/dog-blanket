...
const Rates = [
    0.25, //16 Philpot          x1.05
    0.30, //17 Davis            x1.10
    0.35, //19 Havel            x1.30
    0.40, //18 English Williams x1.60
    0.45, //14 Fitz             x1.80
    0.45, //20 STEM Williams    x2.00
    0.50, //12 Foster           x2.30
    0.55, //13 Boyd             x2.50
    0.60, //21 Santos           x3.00
    1.00  //10 San Marin        x5.00
];
...
/*=============================================================
                        BUILD HOTBAR
=============================================================*/
HotBar.append(HotBarMenuButton, HotBarMenu, BalanceBetWrapper, BetButtonPlus, SpinButton, BetButtonMinus, allInButton);
...
allInButton.classList.add('all-in-button'); // The class name should match the one in your CSS
allInButton.textContent = 'ALL IN'; // The text inside your button
...
BetButtonPlus.addEventListener(`click`, () => {
    if (Balance > BetSize) {
        BetSize = Math.min(Math.round(BetSize + 100), Balance);
    }
    BetText.innerHTML = `BET $${BetSize}`;
...
BetButtonMinus.addEventListener('click', () => {
    BetSize = Math.max(Math.round(BetSize - 100), 100);
    BetText.innerHTML = `BET $${BetSize}`;
...
SpinButton.addEventListener(`click`, (element) => {
    ...
    if (BetSize > Balance) {
        BetSize = Balance; // Reset bet size to the maximum allowable amount
        BetText.innerHTML = `BET $${BetSize}`; // Update the bet display
    }
    ...
    setTimeout(() => {
        ...
        if(Winnings == 0){
            // If the balance after the spin is 0, halve the initial balance
            if(Balance <= 0){
                Balance = Math.max(Math.floor(initialBalance / 2), 1); // Halve the initial balance, ensure it's at least 1
                BetSize = Math.min(BetSize, Balance); // Adjust bet size if it's now higher than the balance
                BalanceText.innerHTML = `BALANCE $${Balance}`;
                BetText.innerHTML = `BET $${BetSize}`; // Update the bet display
            }
        } else {
        ...
        // Always update the balance display
        BalanceText.innerHTML = `BALANCE $${Balance}`;
    }, SpinSpeed + 450);
});

});
// Assuming you have an 'allInButton' with the id 'allInButton' in your HTML
allInButton.addEventListener('click', () => {
    // Your logic here, e.g., setting the bet size to the total balance
    BetSize = Balance;
    BetText.textContent = `BET $${BetSize}`;
    SpinButton.click(); // If you want to simulate a spin button click
});
